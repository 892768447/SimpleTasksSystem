...
class YxCzCollectionModel(Base):

    @classmethod
    def keys(self):
        return ["gh", "fwhm", "ds", "qx", "yxzx", "yyt",
                "qdbm", "qdlx", "zdjx", "zdch", "zdzs",
                "zdlx", "ywlx", "hdmc", "fsmc", "jfje",
                "syzkqje", "gjk", "yck", "zsyhje", "zsycje",
                "zskje", "zswje", "zsdkqje", "sfydd",
                "bczddh", "blsj", "czgh", "czsj", "ywdm", "jffs",
                "zdxh", "cgj", "dxsthsj", "mdj", "yhsxxxsj",
                "dbyf", "hylx", "hyj", "bhsgjk", "sj", "sfkqbl"
                ]
    # 库名
    __dbname__ = "BossReport".lower()
    # 表名 yxbl_201706
    __tablename__ = "YxCz".lower()

    # 表结构
    id = Column(Integer(), primary_key=True)
    gh = Column(String(100), doc="工号")
    fwhm = Column(String(100), doc="服务号码")
    ds = Column(String(100), doc="地市")
    ...
    bczddh = Column(String(100), doc="被冲正订单号")
    blsj = Column(String(100), doc="操作时间")
    czgh = Column(String(100), doc="操作工号")
    czsj = Column(String(100), doc="冲正时间")
...
class YxCzCollection(BaseCollection):

    Name = "营销业务所有冲正报表"  # 名字
    TmpDir = "datas/tmps/营销业务所有冲正报表"  # tmp目录
    DataDir = "datas/营销业务所有冲正报表"  # data目录

    def getModel(self):
        # 数据库模型
        return YxCzCollectionModel
    ...
    def getTime(self):
        # 删除条件的时间字段
        return YxCzCollectionModel.czsj
...
if __name__ == "__main__":
    ...
    job = YxCzCollection(logger, Config.copy(), Codes, Headers.copy(
